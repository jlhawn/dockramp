...
func TestStateRestore(t *testing.T) {
	// Hash all of the Golden values in one batch and save the result.
	// Then iterate over each again, but save and restore state between each.
	// Finally ensure that the resulting hash is the same.
	c := New()

	for _, shaTest := range golden {
		if _, err := io.WriteString(c, shaTest.in); err != nil {
			t.Fatalf("unable to write to hasher: %s", err)
		}
	}

	onePassSum := c.Sum(nil)
	...
	for _, shaTest := range golden {
		...
		state, err := c.State()
		if err != nil {
			t.Fatalf("unable to get digest state: %s\n", err)
		}

		c.Reset()

		if err = c.Restore(state); err != nil {
			t.Fatalf("unable to restore digest state: %s\n", err)
		}
	}

	stateResetSum := c.Sum(nil)

	if !bytes.Equal(onePassSum, stateResetSum) {
		t.Fatalf("sums not equal: %x and %x", onePassSum, stateResetSum)
	}
