...
var testLayers = []testLayer{
	...
	{
		options: &sizedOptions{1, 1024 * 1024, false, false, false}, // a 1mb file (in memory)
...
type sizedOptions struct {
	...
	isRand   bool
	realFile bool
	varySize bool // Generate files with length <= size.
}
...
func Benchmark9kTar(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		ts, err := newTarSum(bytes.NewReader(buf.Bytes()), true, Version0)
		if err != nil {
			b.Error(err)
			return
		}
...
func Benchmark1mbSingleFileTar(b *testing.B) {
	benchmarkTar(b, sizedOptions{1, 1024 * 1024, true, true, false}, false)
}

// this is a single big file in the tar archive
func Benchmark1mbSingleFileTarGzip(b *testing.B) {
	benchmarkTar(b, sizedOptions{1, 1024 * 1024, true, true, false}, true)
}

// this is 1024 1k files in the tar archive
func Benchmark1kFilesTar(b *testing.B) {
	benchmarkTar(b, sizedOptions{1024, 1024, true, true, false}, false)
}

// this is 1024 1k files in the tar archive
func Benchmark1kFilesTarGzip(b *testing.B) {
	benchmarkTar(b, sizedOptions{1024, 1024, true, true, false}, true)
}
